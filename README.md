...
- **Assumption (User Provisioning)**: For security reasons, this is an internal financial system. There is no open "public registration" route. All regular users must be securely provisioned inside the system by an `Admin`.
...
### 🔑 Default Evaluator / Demo Credentials
Since user registration is strictly locked behind Admin privileges, you can log in to the Swagger UI (`POST /api/auth/login`) using the dummy credentials generated by the seed script:
